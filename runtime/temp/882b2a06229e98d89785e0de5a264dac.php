...
<div class="panel panel-default">
    <div class="panel-footer clearfix ">
        <div class="pull-left btn-group hidden-xs" >
            <a href="<?php echo url('control_add'); ?>" js-title="新增" class="btn btn-default js-window-load" js-title="新增" js-unique="true" js-width="60%" js-height="80%">
                <i class="fa fa-plus" aria-hidden="true"></i>&nbsp;新增
            </a>
            <a href="<?php echo U('del_control'); ?>" class="btn btn-outline btn-default js-del-btn del-all" text="删除后将无法恢复，请谨慎操作">
        ...
        <div class="pull-right">
            <form method="get">
                ...
                <input type="text" name="control_name" class="form-control" id="control_name" value="<?php echo input('control_name'); ?>" placeholder="控制端型号">
...
<div class="table-responsive">
        ...
        <?php if(count($data) > 0): if(is_array($data) || $data instanceof \think\Collection || $data instanceof \think\Paginator): if( count($data)==0 ) : echo "" ;else: foreach($data as $key=>$vo): ?>
                <tr>
                    <td><input type="checkbox" class="i-checks" value="<?php echo $vo['control_id']; ?>" name="input[]"></td>
                    <td>
                        <span class="btn-group">
                            <a href="<?php echo url('control_edit',array('id'=>$vo['control_id'])); ?>" class="btn btn-default btn-outline btn-xs js-window-load" title="编辑"><i class="fa fa-edit fa-fw"></i><span class="hidden-xs">编辑</span></a>
                            ...
                            <a href="<?php echo U('printing_edit',['id'=>$vo['control_id'],'status'=>$key,'update_status'=>1]); ?>" js-color="#eea236" class="btn btn-default btn-outline btn-xs js-del-btn" text="启用后可以正常展示"><i class="fa <?php if($vo['status'] == 1): ?>fa-times<?php else: ?>fa-check<?php endif; ?> fa-fw"></i><span class="hidden-xs"><?php echo $item; ?></span></a>
                            <?php endif; endforeach; endif; else: echo "" ;endif; ?>
                            <a href="<?php echo url('del_control',['id'=>$vo['control_id']]); ?>" class="btn  btn-danger btn-outline btn-xs js-del-btn" text="删除后将无法恢复,请谨慎操作！"><i class="fa fa-trash-o fa-fw"></i><span class="hidden-xs">删除</span></a>
                        </span>
                    </td>
