<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:62:"D:\phpStudy\WWW\catering\public\theme\admin\opinion\index.html";i:1512976419;s:62:"D:\phpStudy\WWW\catering\public\theme\admin\layout\layout.html";i:1502172025;s:62:"D:\phpStudy\WWW\catering\public\theme\admin\layout\static.html";i:1503460468;s:58:"D:\phpStudy\WWW\catering\public\theme\admin\layout\js.html";i:1503460468;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <form role="form" action="<?php echo url('index'); ?>" class="panel-body hidden-xs form-inline">
        <div class="form-group">
            <select   class="form-control"  name="opinion_type_id">
                ...
                <?php if(is_array($types) || $types instanceof \think\Collection || $types instanceof \think\Paginator): $i = 0; $__LIST__ = $types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <option value="<?php echo $vo['id']; ?>" <?php if(input('opinion_type_id') == $vo['id']): ?>selected<?php endif; ?> ><?php echo $vo['name']; ?></option>
        ...
        <div class="form-group">
            <select   class="form-control"  name="status">
                ...
                <option value="2" <?php if(input('status') ==2): ?>selected<?php endif; ?>>已处理</option>
                <option value="1" <?php if(input('status') ==1): ?>selected<?php endif; ?>>不予处理</option>
                <option value="0" <?php if(input('status') ==0 && input('is_sale')!=null): ?>selected<?php endif; ?>>未处理</option>
            </select>
        </div>
         <div class="form-group group1">
           <input type="text" name="start_time" class="form-control i-datestart"  id="date2" placeholder="反馈日期(起)" value="<?php echo input('start_time'); ?>">
        </div>
        <div class="form-group group2">
           <input type="text" name="end_time" class="form-control i-dateend" id="date3" placeholder="反馈日期(末)" value="<?php echo input('end_time'); ?>">
        </div>
...
<div class="table-responsive">
    ...
    <tbody>
		...
		<tr>
			...
			<td>
                <?php switch($vo['status']): case "0": ?>待处理<?php break; case "1": ?>不予处理<?php break; case "2": ?>已处理<?php break; default: ?>未知
                <?php endswitch; ?>
            </td>
            ...
            <td>
                ...
                <span class="btn-group">
                    <a href="<?php echo url('set_status',['id'=>$vo['id'],'status'=>1]); ?>" class="btn btn-danger btn-outline btn-xs js-del-btn" text="操作后将无法恢复，请谨慎操作"><i class="fa fa-edit fa-fw"></i><span class="hidden-xs">不予处理</span></a>
                    <a href="<?php echo url('set_status',['id'=>$vo['id'],'status'=>2]); ?>" class="btn btn-danger btn-outline btn-xs js-del-btn" text="操作后将无法恢复，请谨慎操作"><i class="fa fa-edit fa-fw"></i><span class="hidden-xs">处理</span></a>
                </span>
                <?php else: ?>
                <span class="btn-group">
