...
$(function(){
    ...
    var checkInput = checkForm.find('input[js-check],select[js-check]');
    if (checkInput.length>0) {
        checkInput.each(function (k,v) {
            if($(v).val()==''){
                checkInput.attr('js-check-status',false);
                checkForm.find('.js-submit-btn').addClass('disabled');return;
            }
        })
    }
    ...
    $('.js-ajax-form input[js-check]').on('keyup blur',function() {
        ...
        if (form.find('input[js-check-status="false"]').length<=0 && form.find('select[js-check-status="false"]').length<=0) {
    ...
    $('.js-ajax-form select[js-check]').on('change',function() {
        var select = $(this);
        var form  = $(this).parents('.js-ajax-form');
        var tag   = '.js-ajax-form select[name="'+select.attr('name')+'"]';
        var func  = select.attr('js-check');
        if (typeof window[func] == 'function') {
            var re = window[func](this);
            if (re != true) {
                $(tag).attr('js-check-status',false);
